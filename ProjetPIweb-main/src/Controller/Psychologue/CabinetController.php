...
final class CabinetController extends AbstractController
{
    ...
    public function show(Cabinet $cabinet, DisponibiliteRepository $disponibiliteRepository): Response
    {
        return $this->render('cabinet/show.html.twig', [
            ...
            'index_route'    => 'app_psy_cabinet_index',
            'ratings'        => $cabinet->getRatings()->toArray(),
        ]);
    }
    ...
    {
        if ($cabinet->isArchive()) {
            $this->addFlash('error', 'Ce cabinet a ete refuse par l administrateur. Modification impossible.');
    ...
    {
        if ($cabinet->isArchive()) {
            $this->addFlash('error', 'Ce cabinet a ete refuse par l administrateur. Suppression impossible.');
