...
class GeminiService
{
    ...
    public function getActivityAdvice(string $titre, string $description): string
    {
        if (empty($this->apiKey) || str_contains($this->apiKey, 'votre_clé')) {
            return "Conseil IA non disponible (clé API non configurée).";
        }

        $url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=" . $this->apiKey;
        $prompt = "Tu es un expert en bien-être et psychologie. Donne un conseil pratique et bienveillant en français pour cette activité : \"$titre\". Description : \"$description\". Réponds en 2-3 phrases maximum.";
        ...
        try {
            ...
            $data = $response->toArray();
            return $data['candidates'][0]['content']['parts'][0]['text'] ?? "Conseil IA non disponible.";
        } catch (\Exception $e) {
            return "Conseil IA temporairement indisponible.";
        }
    }
